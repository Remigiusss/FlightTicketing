...
public class ScheduleDao implements Dao<Schedule, Integer> {
    ...
    public Collection<ScheduleDTO> searchSchedule(long departureId, long arrivalId, Date departureDate){
        ...
        String sql = "SELECT schedule.*, location_departure.name departure,location_arrival.name arrival " + "from schedule inner join location location_departure " + "on schedule.departure_id = location_departure.id " + "inner join location location_arrival on schedule.arrival_id = location_arrival.id " + " where departure_id = ? and arrival_id = ? and departure_date::timestamtz::date = ?";
        connection.ifPresent(conn -> {
            try {
                PreparedStatement ps = conn.prepareStatement(sql);
                ps.setLong(1, departureId);
                long arrivalIdId = 0;
                ps.setLong(2, arrivalIdId);
